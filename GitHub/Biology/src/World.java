...
//        button.addActionListener(actionListener);
    ...
    public void paint(Graphics g) {
        ...
        g.drawRect(49, 49, simulation.width * SIZE + 1, simulation.height * SIZE + 1);
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (matrix[x][y] == null) {
                    ...
                    g.fillRect(50 + x * SIZE, 50 + y * SIZE, SIZE, SIZE);
                } else if ((matrix[x][y].alive == Bot.LV_ORGANIC_HOLD) || (matrix[x][y].alive == Bot.LV_ORGANIC_SINK)) {
                    g.setColor(new Color(200, 200, 200));
                    g.fillRect(50 + x * SIZE, 50 + y * SIZE, SIZE, SIZE);
                    organic = organic + 1;
                } else if (matrix[x][y].alive == Bot.LV_ALIVE) {
                    g.setColor(Color.BLACK);
                    g.drawRect(50 + x * SIZE, 50 + y * SIZE, SIZE, SIZE);
...
//                    g.setColor(new Color(matrix[x][y].c_red, matrix[x][y].c_green, matrix[x][y].c_blue));
                    g.fillRect(50 + x * SIZE + 1, 50 + y * SIZE + 1, SIZE-1, SIZE-1);
...
//            sleep();        //пауза между ходами
    ...
    public void print() {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (matrix[x][y] == null) {
                    out = " . ";
                } else if (matrix[x][y].alive == Bot.LV_ORGANIC_SINK) {
                    out = " x ";
                } else {
    ...
    public void generateAdam() {
        ...
        bot.alive = Bot.LV_ALIVE;   // отмечаем, что бот живой
